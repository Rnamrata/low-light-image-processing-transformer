...
def remove_up_to_last_slash(input_string):
...
def metrics(im_dir, label_dir, use_GT_mean):
    ...
    dataset1_path = remove_up_to_last_slash(im_dir)
    dataset2_path = remove_up_to_last_slash(label_dir)
    fid_value = calculate_fid(load_images(dataset1_path), load_images(dataset2_path), use_multiprocessing=False, batch_size=8)
            
    if np.isnan(fid_value):
        print(f"FID value calculation returned NaN")
    else:
        fid = fid_value
    ...
    return fid, avg_psnr, avg_ssim, avg_lpips
...
if __name__ == '__main__':
    ...
    fid, avg_psnr, avg_ssim, avg_lpips = metrics(im_dir, label_dir, mea.use_GT_mean)
