...
def eval(model, testing_data_loader, model_path, output_folder,norm_size=True,LOL=False,v2=False,unpaired=False,alpha=1.0):
    ...
    for batch in tqdm(testing_data_loader):
        ...
        print(f"Saving to: {output_folder}")  # Debug print
        output = torch.clamp(output.cuda(),0,1).cuda()
        if not norm_size:
            output = output[:, :, :h, :w]
        
        output_img = transforms.ToPILImage()(output.squeeze(0))
        output_img.save(os.path.join(output_folder, name[0]))  # Use os.path.join for proper path

        print(f"Saved image: {os.path.join(output_folder, name[0])}")  # Debug print
