...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        if iter == train_len:
            ...
            epoch_folder = os.path.join(opt.val_folder, 'training', f'epoch_{epoch}')
            if not os.path.exists(epoch_folder):
                os.makedirs(epoch_folder)
            output_img.save(os.path.join(epoch_folder, 'test.png'))
            gt_img.save(os.path.join(epoch_folder, 'gt.png'))
...
if __name__ == '__main__':  
    ...
    for epoch in range(start_epoch+1, opt.nEpochs + start_epoch + 1):
        ...
        if epoch % opt.snapshots == 0:
            ...
            epoch_output_folder = os.path.join(opt.val_folder, output_folder, f'epoch_{epoch}')
            if not os.path.exists(epoch_output_folder):
                os.makedirs(epoch_output_folder)
            
            print(f"Evaluating epoch {epoch}, saving to {epoch_output_folder}")
            eval(model, testing_data_loader, model_out_path, epoch_output_folder, 
                 norm_size=norm_size, LOL=opt.lol_v1, v2=opt.lolv2_real, alpha=0.8)
            
            im_dir = os.path.join(epoch_output_folder, '*.png')
            images = glob.glob(im_dir)
            print(f"Images found for evaluation: {images}")
            if not images:
                print(f"No images found in {im_dir}. Please check the saving logic.")
                continue

            avg_fid, avg_psnr, avg_ssim, avg_lpips = metrics(im_dir, label_dir, use_GT_mean=False)
            print("===> Avg.FID: {:.4f} ".format(avg_fid))
    ...
    with open("./results/training/metrics.md", "w") as f:
        ...
        f.write("| Epochs | FID | PSNR | SSIM | LPIPS |\n")  
        f.write("|----------------------|----------------------|----------------------|----------------------|\n")  
        for i in range(len(psnr)):
            f.write(f"| {(i+1)*10} | { fid[i]:.4f} | { psnr[i]:.4f} | {ssim[i]:.4f} | {lpips[i]:.4f} |\n")